...
func TestCacheWriteKey(t *testing.T) {
	t.Run("Success", func(t *testing.T) {
		ks := &mocks.KeyStore{}
		ks.On("WriteKey", testKeyID, testKey).Return(nil).Once()
		cache := WithCache(ks, time.Minute)

		err := cache.WriteKey(testKeyID, testKey)
	...
	t.Run("Failure", func(t *testing.T) {
		ks := &mocks.KeyStore{}
		ks.On("WriteKey", testKeyID, testKey).Return(errors.New("no luck")).Once()
		cache := WithCache(ks, time.Minute)
